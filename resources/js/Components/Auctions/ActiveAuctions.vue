<template>
    <ElTable :data="Object.values(auctions)" stripe>
        ...
        <ElTableColumn label="Precio actual" prop="price">
            <template #default="{row}">
                {{row.price}}
            </template>
        </ElTableColumn>
...
<script setup lang="ts">
    ...
    import AuctionDetailDialog from "@/Components/Auctions/AuctionDetail.vue";
    ...
    const viewAuctionDetail = (auction: any) => {
        activeAuction.value = auction
        auctionDetailDialogVisible.value = true
    }

    provide("auctionDetail", { activeAuction, auctionDetailDialogVisible })
</script>
